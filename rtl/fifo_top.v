...
module asynchronous_fifo #(parameter DEPTH=8, DATA_WIDTH=8) (
  ...
  localparam PTR_WIDTH = $clog2(DEPTH); // Changed to localparam for safety
  ...
  wptr_handler #(PTR_WIDTH) wptr_h (
    ...
    .g_rptr_sync(g_rptr_sync), .b_wptr(b_wptr), .g_wptr(g_wptr), .full(full)
  );
  
  rptr_handler #(PTR_WIDTH) rptr_h (
    .rclk(rclk), .rrst_n(rrst_n), .r_en(r_en), 
    .g_wptr_sync(g_wptr_sync), .b_rptr(b_rptr), .g_rptr(g_rptr), .empty(empty)
  );
  
  // Pass all parameters to memory explicitly
  fifo_mem #(.DEPTH(DEPTH), .DATA_WIDTH(DATA_WIDTH), .PTR_WIDTH(PTR_WIDTH)) fifom (
